...
from src.api.schemas import SealDetectionResponse
...
@router.post("/detect", response_model=SealDetectionResponse)
async def detect_seal_text(
    file: UploadFile,
    settings: Settings = Depends(get_settings),
    queue: InferenceQueue = Depends(get_inference_queue),
    hardware: HardwareInfo = Depends(get_hardware_info),
    engine: PaddleXModelEngine = Depends(engine_dependency("seal_detection")),
