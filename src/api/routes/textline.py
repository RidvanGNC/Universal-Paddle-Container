...
from src.api.schemas import TextLineOrientationResponse
...
router = APIRouter(prefix="/textline", tags=["textline"])


@router.post("/orientation", response_model=TextLineOrientationResponse)
async def classify_textline_orientation(
    file: UploadFile,
    settings: Settings = Depends(get_settings),
    queue: InferenceQueue = Depends(get_inference_queue),
    hardware: HardwareInfo = Depends(get_hardware_info),
    engine: PaddleXModelEngine = Depends(engine_dependency("textline_orientation")),
    principal: Principal = Depends(get_current_principal),
) -> TextLineOrientationResponse:
    ...
    return TextLineOrientationResponse(
